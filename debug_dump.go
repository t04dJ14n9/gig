...
func instructionAnnotation(op bytecode.OpCode, operands []byte, prog *bytecode.CompiledProgram) string {
	switch op { //nolint:exhaustive // Only opcodes with useful human-readable metadata need annotations.
	...
	case bytecode.OpChangeType:
		return changeTypeAnnotation(readUint16(operands, 0), readUint16(operands, 2), prog)
	default:
		return ""
	}
}
...
func changeTypeAnnotation(typeIdx, sourceLocal uint16, prog *bytecode.CompiledProgram) string {
	if sourceLocal == bytecode.NoSourceLocal {
		return fmt.Sprintf("%s, source=<stack>", typeAnnotation(typeIdx, prog))
	}
	return fmt.Sprintf("%s, source=%s", typeAnnotation(typeIdx, prog), localAnnotation(sourceLocal))
}
...
func describeConstant(v any) string {
	...
	case *external.ExternalFuncInfo:
		return fmt.Sprintf("external func %T direct=%t", c.Func, c.DirectCall != nil)
	case *external.ExternalMethodInfo:
		return fmt.Sprintf("external method %s direct=%t receiver=%s", c.MethodName, c.DirectCall != nil, c.ReceiverTypeName)
	default:
