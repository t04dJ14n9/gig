...
1. **Gig 在性能上全面超越 gofun 和 Yaegi**，递归场景快 5.4 倍，外部调用快 2.8 倍
...
- **OS**: Linux amd64
- **Go 版本**: 1.23.1
- **统计方法**: `-count=5` 配合 `benchstat`

### 核心工作负载对比

| 工作负载 | 原生 Go | Gig | gofun | Yaegi | Gopher-Lua | 规则引擎 | 备注 |
|----------|---------|-----|-------|-------|------------|----------|------|
| **Fibonacci(25)** 递归 | 449 μs | **20.1 ms** | ❌ 不支持 | 109 ms | 6.8 ms | ❌ 不支持 | gofun 不支持递归 |
| **Fibonacci(25)** 迭代 | 492 μs | - | **17.4 ms** | - | - | ❌ 不支持 | gofun 迭代版本 |
| **ArithmeticLoop(1K)** | 676 ns | **36.3 μs** | 330 μs | 40.9 μs | 18 μs | ❌ 不支持 | Gig 快 9.1 倍 vs gofun |
| **ExternalCall(100)** | 61 ns | **34 μs** | 245 μs | 100 μs | N/A | ❌ 不支持 | Gig 快 7.2 倍 vs gofun |
| **Closure(100)** | 34 ns | **35.2 μs** | 196 μs | 1 ms | 156 μs | ❌ 不支持 | Gig 快 5.6 倍 vs gofun |
| **SliceOps(100)** | 69 ns | **12.5 μs** | 84.5 μs | - | - | ❌ 不支持 | Gig 快 6.8 倍 vs gofun |
| **MapOps(100)** | 10.1 μs | **112 μs** | 153 μs | - | - | ❌ 不支持 | Gig 快 1.4 倍 vs gofun |
| **简单条件判断** | 0.34 ns | **0.69 μs** | 3.2 μs | 1.5 μs | N/A | ~1.6 μs | Gig 快 4.7 倍 vs gofun |
| **解析性能** | - | 124 μs | **16.2 μs** | - | - | - | gofun 解析更快 |

### gofun vs Gig 详细性能对比

| 测试场景 | 原生 Go | Gig | gofun | Gig/gofun 比值 | Gig 优势 |
|----------|---------|-----|-------|----------------|----------|
| **Fibonacci(25)** 迭代 | 492 μs | 22.5 ms | 17.4 ms | 0.77x | gofun 快（迭代优化） |
| **ArithmeticLoop(1K)** | 676 ns | 36.3 μs | 330 μs | **9.1x** | ✅ Gig 显著领先 |
| **ExternalCall(100)** | 61 ns | 34 μs | 245 μs | **7.2x** | ✅ Gig 显著领先 |
| **Closure(100)** | 34 ns | 35.2 μs | 196 μs | **5.6x** | ✅ Gig 显著领先 |
| **Condition** | 0.34 ns | 689 ns | 3.2 μs | **4.7x** | ✅ Gig 显著领先 |
| **VariableOps** | 0.33 ns | 584 ns | 2.9 μs | **5.0x** | ✅ Gig 显著领先 |
| **SliceOps(100)** | 69 ns | 12.5 μs | 84.5 μs | **6.8x** | ✅ Gig 显著领先 |
| **MapOps(100)** | 10.1 μs | 112 μs | 153 μs | **1.4x** | ✅ Gig 领先 |
| **ParseOnly** | - | 124 μs | 16.2 μs | 0.13x | gofun 解析更快 |
...
| ExternalCall | **305** | 1,867 | 少 6.1 倍 |
| Closure | **195** | 2,210 | 少 11.3 倍 |
| SliceOps | **10** | 1,422 | 少 142 倍 |
| MapOps | **1,223** | 1,846 | 少 1.5 倍 |
...
|----------|--------------|----------------|------------------|------|
| Fibonacci(25) | **7** | 2,138,703 | ❌ 不支持 | Gig 少 305,529 倍 |
| BubbleSort(100) | **9** | 5,085 | ❌ 不支持 | Gig 少 565 倍 |
| Sieve(1000) | **7** | 43 | ❌ 不支持 | Gig 少 6 倍 |
| 简单规则判断 | ~9 allocs | ~50 allocs | **~17 allocs** | 规则引擎中等 |
| 嵌套条件判断 | ~10 allocs | ~100 allocs | **~39 allocs** | 规则引擎中等 |
| 外部函数调用 | ~305 allocs | ~1000 allocs | **~17 allocs** | 规则引擎更优 |
...
1. **递归快 5.4 倍**：O(1) 函数查找、帧池化、仅 7 次分配 vs 210 万次
2. **外部调用快 2.6-2.8 倍**：1,162 个生成的 DirectCall 包装器消除 92% 标准库函数的 `reflect.Value.Call()`
3. **紧凑循环快 1.1-1.3 倍**：整数特化 `int64` 局部变量和融合超级指令

**Gopher-Lua 为何在核心工作负载上更快**：
...
|----------|-----------|-----|----------|----------|
| 简单条件判断 | 0.7 μs | 0.9 μs | **1.6 μs** | `tests/robustness_comparison_test.go` |
| 嵌套条件判断 | 0.5 μs | 1.0 μs | **4.0 μs** | `tests/robustness_comparison_test.go` |
| 算术循环(1K) | 0.7 μs | 50.3 μs | ❌ 不支持 | `tests/robustness_comparison_test.go` |
| 外部函数调用 | 62 ns | 32 μs | **1.6 μs** | `tests/robustness_comparison_test.go` |
| Fibonacci(20) | 41 μs | 1.85 ms | ❌ 不支持 | `tests/robustness_comparison_test.go` |
