...
- `WithAllowPanic()` —— 默认下 `panic()` 在编译期就被
  ...
  按 Go 语义工作。
- `WithStatefulGlobals()` —— 已是空操作，保留为源码兼容。
...
`value.DefaultConverter().FromAny`），结果反向。`RunWithValues` 形式跳过
两次转换，给已经持有 `[]value.Value` 的调用方使用。
...
3. 构造每次调用的 `frame`：SSA 函数指针、当前/前驱基本块、`cells` 表
   （`ssa.Value → *Cell`）、闭包的自由变量 cell 列表。
4. 绑定形参与自由变量。
...
`AddFunction` 接收的是真正的 `func` 值（不是 wrapper），运行时通过
`reflect.Call` 分发。`AddVariable` 接收指针，所以读经由 `UnOp(MUL)` 加载、
写可以 `Set` 到槽位。`AddType` 注册 `reflect.Type` 给类型检查器解析。
...
3. 否则查 `LookupFunc(pkg, fn.Name())`，调用其 `Function.Call`。
4. 若查找失败，再尝试一次 `invokeMethodOn` —— 覆盖少数没把方法注册成自由
   函数的标准库包。

返回的 `host.Function` 是 `reflectFunc{fn: reflect.ValueOf(rawFn)}`。它的
`Call` 通过 `Converter.ToReflect` 构造 reflect 形式的实参，经由
`reflect.Value.Call` 分发（变长且预先打包成 slice 时用 `CallSlice`）。
...
1. 函数指针就是真正的 `strings.ToUpper`，没合成的 wrapper。运行时分发只走
   reflect。
2. `AddFunction` 签名是 `(name, fn, doc)` —— 三个参数。没有"快路径"注册了
   （legacy DirectCall 路径已与字节码 VM 一起删除；理由见 `docs/PLAN.md`）。
...
`RunWithContext(ctx, ...)` 直接使用调用方提供的 context。当前解释器只在
前端流水线的阶段之间检查 `ctx.Err()`（`builder.go` 在 parse 之前以及
类型检查与 SSA 之间各检查一次）。SSA 调度循环内部不做轮询 —— 进入紧
死循环的脚本不会响应取消，直到外层 goroutine 被测试 runner 杀掉。这是
已知的不足；按每 N 条指令检查 `ctx.Done()` 是后续工作。
