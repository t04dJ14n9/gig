...
// need wrapping with sanitizeArgForFmt to properly format interpreter structs.
// Currently only "fmt" needs this (for _gig_id hiding, %T, and String() dispatch).
var fmtSanitizePackages = map[string]bool{
	"fmt": true,
}

// fmtSanitizeExtraImports returns additional imports needed by the generated file
// when fmtSanitize helpers are emitted into the package.
func fmtSanitizeExtraImports() map[string]string {
...
// fmtSanitizeHelperCode returns the Go source code for fmt argument sanitization
// helpers. This code is emitted directly into the generated fmt.go file so that
// the DirectCall wrappers can call sanitizeArgForFmt and sprintfWithTypeAwareness.
func fmtSanitizeHelperCode() string {
	return `// --- fmt sanitization helpers (generated by gentool) ---
...
// isFmtSprintfLike checks if a function has the signature pattern:
...
// the generated code uses sprintfWithTypeAwareness to handle %T correctly.
func isFmtSprintfLike(fi *funcInfo) bool {
	sig := fi.Sig
	if !sig.Variadic() {
		return false
	}
...
func generateDirectCall(fi *funcInfo, pkgRef string) string {
	...
	needsSanitize := fmtSanitizePackages[currentPkgPath]
	...
	// For fmt.Sprintf-like functions (string, ...interface{}) string, use
	// sprintfWithTypeAwareness to correctly handle %T with interpreter structs.
	var callExpr string
	if needsSanitize && isFmtSprintfLike(fi) {
		callExpr = fmt.Sprintf("sprintfWithTypeAwareness(%s)", strings.Join(argExprs, ", "))
	} else {
