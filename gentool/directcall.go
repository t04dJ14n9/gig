...
// customDirectCalls contains hand-written DirectCall overrides for functions that
// need special handling beyond what the automatic generator can produce.
// Key format: "pkg_path.FuncName" (e.g., "sort.Ints").
var customDirectCalls = map[string]string{
	// sort.Ints: writeback wrapper for []int64 internal representation.
	// The VM stores []int as []int64; Interface() creates a []int copy.
	// After sorting the copy, we write sorted values back to the original []int64.
	"sort.Ints": `func direct_sort_Ints(args []value.Value) value.Value {
...
`,
	// sort.Float64s: no writeback needed since []float64 is stored as reflect.Value.
	"sort.Float64s": `func direct_sort_Float64s(args []value.Value) value.Value {
	a0 := args[0].Interface().([]float64)
	sort.Float64s(a0)
	return value.MakeNil()
}
`,
	// fmt.Sprintf: intercept %T for gig structs (fmt.Formatter doesn't handle %T).
	"fmt.Sprintf": `func direct_fmt_Sprintf(args []value.Value) value.Value {
	a0 := args[0].String()
...
// fmtSanitizerPreamble is injected at the top of fmt.go before the init() function.
// It provides gigStructFormatter to properly format interpreter-synthesized structs.
const fmtSanitizerPreamble = `
...
func generateDirectCall(fi *funcInfo, pkgRef string) string {
	...
	if isVariadic {
		...
		if isEmptyInterface(elemType) {
			...
			// For the fmt package, use sanitizeArgForFmt to properly handle
			// interpreter-synthesized structs (_gig_id, Stringer, %T).
			if currentPkgPath == "fmt" {
				b.WriteString(fmt.Sprintf("\t\tvarArgs[i-%d] = sanitizeArgForFmt(args[i])\n", fixedCount))
			} else {
